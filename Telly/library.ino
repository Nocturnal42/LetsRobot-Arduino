...
int nextarg_int(int min, int max, int default_val, int on_error) {
    ...
    if (! arg)
        return default_val;
    //Serial.println(format("range: %d<%s<%d", min, arg, max));
    
    //Serial.println(format("nextarg_int: %s", arg));
    val = strtol(arg, &endptr, 0);
    //Serial.println(format("val: %d", val));
    ...
    if (! (min <= val && val <= max)) {
        //Serial.println(format("Out of range: %d<%d<%d", min, val, max));
        return on_error;
    }
...
void dispatch(char *line) {
    ...
    ERR(format("Unknown command %s", line));
}
...
void compat_loop() {
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'b': do_command("backward");    break; 
